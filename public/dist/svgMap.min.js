...
.then(countries => {
  ...
  for (i = 0; i < countries.length; i++) {
    if (countries[i].countryInfo.iso2 !== null) {
      svgMapCountries.values[countries[i].countryInfo.iso2] = {population: countries[i].population, infected: countries[i].cases,
      deaths: countries[i].deaths, recovered: countries[i].recovered, active: countries[i].active, critical: countries[i].critical, tests: countries[i].tests, last_updated: new Date(countries[i].updated).toDateString() }; 
    }
  };
  new svgMap({
      targetElementID: 'svgMap',
      data: svgMapCountries
  });
});