...
function FetchingContinentDataFromCOVID_19_API() {
    ...
    .then(list_of_continents => {
        ...
        var query = Continent.countDocuments({}, (err, count) => {return count;});
        query.exec().then((count) => {
            continent_length = count;
        var queryData = Continent.find({}, (err, continents) => {return continents;});
        queryData.exec().then((continents) => {
            continents_arr = continents;
        if (continent_length === 0) {
            continents_arr = InsertingDocumentsToContinentCollection(list_of_continents);
            ...
            for (i = 0; i < list_of_continents.length; i++) { 
                if (list_of_continents[i].updated > (continents_arr[i].last_update + 599999)) {
                    Continent.updateOne(
                    { continent: list_of_continents[i].continent },
                    {
                        $set: { continent: list_of_continents[i].continent,
    ...
    .then(list_of_countries => {
        ...
        if (countries_length === 0) {
            ...
            for (i = 0; i < list_of_countries.length; i++) { 
                ...
                if (list_of_countries[i].updated > continents_arr[indexForContinent].countries[indexForCountry].last_update && continents_arr[indexForContinent].continent !== 'Unknown') {
                    Continent.updateOne(
                    { 'countries.country': list_of_countries[i].country },
                    {
                        $set: { 'countries.$.country': list_of_countries[i].country,
                                'countries.$.countryInfo.iso2': list_of_countries[i].countryInfo.iso2,
                                'countries.$.countryInfo.iso3': list_of_countries[i].countryInfo.iso3,
                                'countries.$.countryInfo.lat': list_of_countries[i].countryInfo.lat,
                                'countries.$.countryInfo.long': list_of_countries[i].countryInfo.long,
                                'countries.$.countryInfo.flag': list_of_countries[i].countryInfo.flag,
                                ...
                                'countries.$.todayRecovered': list_of_countries[i].todayRecovered,
                                'countries.$.active': list_of_countries[i].active,
                                'countries.$.critical': list_of_countries[i].critical,
                                'countries.$.casesPerOneMillion': list_of_countries[i].casesPerOneMillion,
                                'countries.$.deathsPerOneMillion': list_of_countries[i].deathsPerOneMillion,
                                'countries.$.recoveredPerOneMillion': list_of_countries[i].recoveredPerOneMillion,
                                'countries.$.tests': list_of_countries[i].tests,
                                'countries.$.testsPerOneMillion': list_of_countries[i].testsPerOneMillion,
                                'countries.$.population': list_of_countries[i].population,
                                'countries.$.continent': list_of_countries[i].continent,
                                'countries.$.oneCasePerPeople': list_of_countries[i].oneCasePerPeople,
                                'countries.$.oneDeathPerPeople': list_of_countries[i].oneDeathPerPeople,
                                'countries.$.oneTestPerPeople': list_of_countries[i].oneTestPerPeople,
                                'countries.$.activePerOneMillion': list_of_countries[i].activePerOneMillion,
                                'countries.$.criticalPerOneMillion': list_of_countries[i].criticalPerOneMillion,
                ...
                else if (list_of_countries[i].updated > continents_arr[indexForContinent].countries[indexForCountry].last_update && continents_arr[indexForContinent].continent === 'Unknown') {
                    Continent.updateOne(
                        { 'countries.country': list_of_countries[i].country },
                        {
                            $set: { 'countries.$.country': list_of_countries[i].country,
            ...
            Continent.aggregate([
                ...
                { $group : {
                    ...
                    recoveredPerOneMillion: {$sum : "$countries.recoveredPerOneMillion"},
                    ...
                    criticalPerOneMillion: {$sum : "$countries.criticalPerOneMillion"},
                }}
                ...
                Continent.updateOne(
                    ...
                    {
                        $set: { continent: continents_arr[indexForContinent].continent,
                                ...
                                recoveredPerOneMillion: unknownContinent[0].recoveredPerOneMillion,
                                ...
                                criticalPerOneMillion: unknownContinent[0].criticalPerOneMillion,
                                last_update: new Date().getTime()
                             }
                    },
                    { upsert: true }, function(err, res) {
            ...
            for (i = 0; i < list_of_countries.length; i++) {
                ...
                if (list_of_countries[i].continent === '') {
                    ...
                    Continent.updateOne(
                        { continent: 'Unknown' },
                        {
                            $inc: { cases: list_of_countries[i].cases,
                                    todayCases: list_of_countries[i].todayCases,
                                    deaths: list_of_countries[i].deaths,
                                    todayDeaths: list_of_countries[i].todayDeaths,
                                    recovered: list_of_countries[i].recovered,
...
module.exports.FetchingContinentDataFromCOVID_19_API = FetchingContinentDataFromCOVID_19_API;